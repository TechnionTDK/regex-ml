...
def split_into_sentences(text):
    ...
    No fancy NLP here, only separate on "dot" signs.
    """
...
def load_dataset():  # Load "Torat Emet" dataset
    ...
    for x in range(SAMPLE_SIZE):
        progress = progress + 1
        print(f"Progress: |{(progress/SAMPLE_SIZE)*100}%|")
        data = df['text'][x]
    ...
    # adding column to df with ngram indices(regardless of their sentences), for filtering uses
